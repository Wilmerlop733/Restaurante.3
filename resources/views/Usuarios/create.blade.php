...
<head>
    ...
    <title>{{ __('Crear Usuario') }} - {{ __('Restaurante') }}</title>
            ...
            <div class="col-md-6">
                <div class="card border-0 shadow">
                    <div class="card-header bg-primary text-white py-3">
                        <h5 class="mb-0"><i class="bi bi-person-plus-fill"></i> {{ __('Crear Usuario Nuevo') }}</h5>
                    </div>
                    <div class="card-body p-4">
                        <form action="{{ route('usuarios.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-bold">{{ __('Contraseña') }}</label>
                                <input type="password" name="password" class="form-control @error('password') is-invalid @enderror" required>
                                @error('password') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>

                            <div class="mb-3">
                                <label class="form-label fw-bold">{{ __('Confirmar Contraseña') }}</label>
                                <input type="password" name="password_confirmation" class="form-control" required>
                            </div>

                            <div class="mb-4">
                                <label class="form-label fw-bold">{{ __('Asignar Rol') }}</label>
                            ...
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-primary w-100 py-2 shadow-sm">{{ __('Crear Usuario') }}</button>
